...
The updater is now a thin wrapper over `tempoch-core`'s shared
`runtime_data` bundle builder, so compile-time regeneration and the optional
runtime-refresh feature use the same fetch/parse/build pipeline.
...
Separately, `tempoch` now also offers an opt-in runtime-refresh path behind
the `runtime-data` feature. The updater exists to refresh the checked-in,
...
- If you want runtime refresh inside an application instead of regenerating
  checked-in files, use `tempoch::runtime_data` with the `runtime-data`
  feature and `TEMPOCH_DATA_DIR` for cache placement.
