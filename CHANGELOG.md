...
- Prepared `tempoch-ffi` for crates.io publication against `tempoch` and
  `tempoch-core` 0.6.5, and synchronized its reported ABI version.
- Relaxed `tempoch-ffi` dependency requirements to compatible release lines.

## [0.6.5] - 2026-06-21

### Changed

- Updated `affn` to 0.8 and aligned the workspace on `qtty` 0.8.5.
- Updated `siderust-archive` to 0.1.4 for the latest bundled time-data support.
- Aligned the FFI build dependencies on `qtty-ffi` 0.8.2 and `cbindgen`
  0.29.3, and synchronized the reported FFI ABI version with this release.
- Updated `chrono` dependencies to 0.4.44.
