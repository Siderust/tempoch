...
//! - `tai_minus_utc()` offset at a given JD
...
fn main() {
    ...
    // In UTC it is 2000-01-01T11:58:56Z (ΔT ≈ 63.8 s at that epoch).
    ...
    let tai_utc = tai_minus_utc(jd.value()); // TAI − UTC in seconds
    ...
    // and TAI − UTC = tai_minus_utc(jd) (increases with leap seconds).
    ...
    let tai_utc_s = tai_minus_utc(jd.value()) as i32;
