...
//! Interchange format showcase: every way to construct and read a `Time<A>`.
//!
//! Sections:
//!   1. SI seconds (J2000-TT offset) — all continuous axes
//!   2. Julian Day — all continuous axes
//!   3. Modified Julian Day — all continuous axes
//!   4. POSIX / Unix seconds — UTC axis only
//!   5. GPS seconds — TAI axis only
//!   6. chrono `DateTime<Utc>` — UTC axis only
//!
//! Run with:
//! ```sh
...
fn main() {
    ...
    //
    // The native internal representation of every continuous-axis instant.
    // Zero corresponds to J2000.0: 2000-01-01T12:00:00 TT.
    // Negative values are before J2000; positive after.
    // Available on: TT, TAI, TDB, TCG, TCB, UT1.
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // JD = 2 451 545.0 at J2000.0.  Fractions denote time-of-day (0.5 = noon).
    // Available on: TT, TAI, TDB, TCG, TCB, UT1.
    // ─────────────────────────────────────────────────────────────────────────
    let j2000_from_jd = Time::<TT>::from_julian_days(J2000_JD_TT).unwrap();
    let unix_epoch_tt = Time::<TT>::from_julian_days(UNIX_EPOCH_JD).unwrap();
    let half_day_tt = Time::<TT>::from_julian_days(Day::new(2_451_545.5)).unwrap();

    println!();
    println!("── 2. Julian Day ─────────────────────────────────────────────");
    println!("  J2000 JD(TT)   : {:.9}", j2000_from_jd.julian_days());
    println!("  Unix epoch JD  : {:.9}", unix_epoch_tt.julian_days());
    println!("  JD + 0.5 (noon): {:.9}", half_day_tt.julian_days());
    println!("  Round-trip SI  : {:.3}", j2000_from_jd.si_seconds());

    // ─────────────────────────────────────────────────────────────────────────
    // 3. Modified Julian Day (MJD)
    //
    // MJD = JD − 2 400 000.5.  Starts at midnight on 1858-11-17.
    // More compact and convenient for modern dates than JD.
    // Available on: TT, TAI, TDB, TCG, TCB, UT1.
    // ─────────────────────────────────────────────────────────────────────────
    let j2000_from_mjd = Time::<TT>::from_modified_julian_days(Day::new(51_544.5)).unwrap();
    let unix_epoch_from_mjd = Time::<TT>::from_modified_julian_days(UNIX_EPOCH_MJD).unwrap();

    println!();
    println!("── 3. Modified Julian Day (MJD) ──────────────────────────────");
    println!("  J2000 MJD(TT)  : {:.9}", j2000_from_mjd.modified_julian_days());
    println!("  Unix epoch MJD : {:.9}", unix_epoch_from_mjd.modified_julian_days());
    println!("  JD − MJD       : {:.1}", j2000_from_jd.julian_days() - j2000_from_jd.modified_julian_days());

    // ─────────────────────────────────────────────────────────────────────────
    // 4. POSIX / Unix seconds  (UTC axis only)
    //
    // Counts seconds from 1970-01-01T00:00:00 UTC, ignoring leap seconds
    // (every "Unix day" is exactly 86 400 ticks regardless of leap seconds).
    // Matches C `time()`, Python `time.time()`, JavaScript `Date.now()/1000`.
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // GPS = TAI − 19 s (exact, fixed at launch).  No leap-second
    // corrections: GPS and TAI share the same tick rate.
    // ─────────────────────────────────────────────────────────────────────────
    let gps_epoch_tai = Time::<TAI>::from_gps_seconds(Second::new(0.0)).unwrap();
    let gps_j2000 = j2000_from_jd.to::<TAI>();
    let now_gps = now_utc.to::<TAI>();
    ...
    // Leap-second instants are encoded as `timestamp_subsec_nanos >= 1e9`
    // in chrono's representation.  `is_leap_second()` recomputes the label
    // from the UTC-TAI segment table, so it is stable after axis round-trips.
    // ─────────────────────────────────────────────────────────────────────────
    ...
    let now_roundtrip = now_utc.to::<TAI>().to::<UTC>();
