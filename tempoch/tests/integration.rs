...
fn ut1_context_roundtrip_near_j2000() {
    ...
    let tt = J2000Seconds::<TT>::new(0.0).to_j2000s();
...
fn public_constats_epochs_are_usable() {
    let j2000 = JulianDate::<TT>::new(J2000_JD_TT_DAY.value()).to_j2000s();
    let tai_s: Time<TAI> = j2000.to::<TAI>();

    assert_eq!(j2000.to::<JD>().raw(), J2000_JD_TT_DAY);
    assert!(
...
fn encoded_date_helpers_and_assign_arithmetic_are_available() {
    let mut jd = JulianDate::<TT>::new(J2000_JD_TT_DAY.value());
    jd += Day::new(2.0);
    jd -= Day::new(0.5);

    assert_eq!(jd.value(), 2_451_546.5);
    assert_eq!(jd.to::<MJD>().value(), jd.value() - 2_400_000.5);
    assert!((JulianDate::<TT>::new(2_451_545.0).julian_centuries()).abs() < 1e-12);

    let mut mjd = ModifiedJulianDate::<TT>::new(51_544.5);
    mjd += Day::new(1.0);
    mjd -= Day::new(0.25);
...
fn encoded_date_min_max_and_mean_are_available() {
    ...
    let b = JulianDate::<TT>::new(2_451_547.0);
...
fn utc_supports_coordinate_views_and_pre_1961_roundtrips() {
    ...
    let utc = J2000Seconds::<UTC>::new(0.0).to_j2000s();
...
fn interval_set_ops_match_expected_intervals() {
    let outer = Period::<TT>::new(
        ModifiedJulianDate::<TT>::new(0.0).to_j2000s(),
        ModifiedJulianDate::<TT>::new(10.0).to_j2000s(),
    );
    let a = vec![
        Period::<TT>::new(
            ModifiedJulianDate::<TT>::new(1.0).to_j2000s(),
            ModifiedJulianDate::<TT>::new(3.0).to_j2000s(),
        ),
        Period::<TT>::new(
            ModifiedJulianDate::<TT>::new(5.0).to_j2000s(),
            ModifiedJulianDate::<TT>::new(9.0).to_j2000s(),
        ),
    ];
    let b = vec![
        Period::<TT>::new(
            ModifiedJulianDate::<TT>::new(2.0).to_j2000s(),
            ModifiedJulianDate::<TT>::new(4.0).to_j2000s(),
        ),
        Period::<TT>::new(
            ModifiedJulianDate::<TT>::new(7.0).to_j2000s(),
            ModifiedJulianDate::<TT>::new(8.0).to_j2000s(),
        ),
    ];
...
fn public_serde_roundtrips_time_and_periods() {
    let tt = J2000Seconds::<TT>::new(12.5).to_j2000s();
    let jd = JulianDate::<TT>::new(2_451_545.25).to_j2000s();
    let mjd_period = Period::<TT>::new(
        ModifiedJulianDate::<TT>::new(61_000.0).to_j2000s(),
        ModifiedJulianDate::<TT>::new(61_001.0).to_j2000s(),
    );

    let tt_json = serde_json::to_value(tt).unwrap();
    assert_eq!(serde_json::from_value::<Time<TT>>(tt_json).unwrap(), tt);

    let jd_json = serde_json::to_value(jd).unwrap();
    assert_eq!(serde_json::from_value::<Time<TT>>(jd_json).unwrap(), jd);

    let period_json = serde_json::to_value(mjd_period).unwrap();
    assert_eq!(
        serde_json::from_value::<Period<TT>>(period_json).unwrap(),
        mjd_period
    );
}
...
fn serde_still_works_with_current_shape() {
    let tt = J2000Seconds::<TT>::new(1.25).to_j2000s();
    let period = Period::<TT>::new(
        J2000Seconds::<TT>::new(1.25).to_j2000s(),
        J2000Seconds::<TT>::new(2.5).to_j2000s(),
    );

    let tt_json = serde_json::to_value(tt).unwrap();
    assert_eq!(serde_json::from_value::<Time<TT>>(tt_json).unwrap(), tt);

    let period_json = serde_json::to_value(period).unwrap();
    assert_eq!(
        serde_json::from_value::<Period<TT>>(period_json).unwrap(),
        period
    );
